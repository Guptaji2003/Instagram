 ...
 const {followhandle}= UseFollow();
  ...
  return (
    <div className="max-w-md mx-auto bg-white border border-gray-300 rounded-md shadow-md">
      ...
      <div className="flex items-center justify-between p-3">
        <div className="flex items-center space-x-3">
          <Link to={`/profile/${post.author?._id}`}>
            <img src={(post.author?.profileimage?.includes("data")? post.author?.profileimage  : IMG)} alt="Profile" className="w-10 h-10 rounded-full " />
          </Link>
          <div className="flex flex-col">
            <h2 className="text-sm font-semibold">{post.author?.username}</h2>
            <h5 className="text-gray-600 font-mono text-xm">{post?.location}</h5>
          </div>
          ...
          {user?._id !== post.author?._id ? (
            ...
            >
              {user?.followings.includes(post.author?._id)
                ? <span className=" text-white bg-gray-500 p-2 rounded-2xl">Following</span>
                : <span className=" text-white bg-blue-500 p-2 rounded-2xl">Follow</span>}
            </button>
          ) : (
            <></>
          )}
          ...
          <div className="relative group">
            ...
            <div className="hidden group-hover:block absolute  -mt-2 -left-10 w-32 bg-white border border-gray-300 rounded-lg shadow-lg">
                <ul>
                   ...
                   {user?._id === post.author?._id && <li onClick={() => deletehandle()} className="px-4 py-2  hover:bg-gray-200 text-red-600 cursor-pointer">Delete</li>}
                </ul>
            </div>
        </div>
        </div>
      ...
      {/* Post Image */}
      <img src={(post.photo?.includes("data")?  post.photo : IMG) } alt="Post" width={400}  />
              ...
              <div className="mt-3 max-h-60 overflow-y-auto">
                {post.comments.map((comment) => (
                  <div key={comment._id} className="p-1 border-b flex ">
                    <img
                      src={(comment.author?.profileimage?.includes("data")? comment.author.profileimage  : IMG)}
                      alt="Profile"
